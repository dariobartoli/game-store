...
const create = async(req,res) => {
    try {
        ...
        let publication = new PublicationModel({title, text, active: true, user:req.user.id})
        ...
        return res.status(201).json({message: "publication has been created successful", publication})
    } catch (error) {
...
const set = async(req,res) => {
    try {
        ...
        const publicationIndex = userLogged.publications.findIndex(element => element.publication._id.toString() === id);
        ...
        return res.status(200).json({message: "post edited", publication: updatedPublication})
    } catch (error) {
...
const delet = async(req,res) => {
    try {
        ...
        const userLogged = await UserModel.findById({_id: req.user.id})
        const publicationIndex = userLogged.publications.findIndex(element => element.publication._id.toString() === filter);
        if (publicationIndex === -1) {
            throw new Error("Publication not found");
        }
